...
-- MAIN EVENTS
-- 
if GS_AC.UseESX then
    TriggerEvent(GS_AC.ESXTrigger, function(obj) ESX = obj end)
        ...
        if _char ~= nil then
            ...
            if _charmoney ~= nil and efectivo ~= nil then
                if tonumber(_charmoney) > tonumber(efectivo) then
                    ...
                    if amount > GS_AC.MaxTransferAmount then 
...
AddEventHandler('PJHxig0KJQFvQsrIhd5h', function(Metadata, Files)
    ...
    if _mdata ~= nil then
        for k,v in pairs(_mdata) do
            if not GS_AC.WhitelistedResources[k] then
        ...
        elseif (_type == "vehiclemodifier") then
            ...
            if GS_AC.UseESX then
                ...
                if type == 1 or type == 2 or type == 3 or type == 4 or type == 5 or type == 6 then
                    ...
                    if _job ~= GS_AC.MechanicJobName then
                        LogDetection(_src, "Tried to modify vehicle features. Type: ".._item,"model")
                        kickandbanuser(" Vehicle Modifier Detected.", _src)
                    end
                end
...
RegisterCommand('reloadacbans', function(source)
    ...
    if _src ~= 0 then
        if IsPlayerAceAllowed(_src, "vbacbypass") or IsPlayerAceAllowed(_src, "vbacadmin") then
            loadBanList()
            TriggerClientEvent('chat:addMessage', _src, {args = {"^*^7[^1GS-AC^7]: BanList Reloaded"}})
        end
    else
        loadBanList()
        print("^*^7[^1GS-AC^7]: BanList Reloaded")
    end
end, false)
...
Citizen.CreateThread(function()
    ...
    while true do
        loadBanList()
        Citizen.Wait(GS_AC.ReloadBanListTime)
    end
end)
...
Citizen.CreateThread(function()
    ...
    particlesSpawned = {}
    while true do
        Citizen.Wait(GS_AC.ReloadEntityTime) -- augment/lower this if you want.
        explosionsSpawned = {}
        vehiclesSpawned = {}
        pedsSpawned = {}
        entitiesSpawned = {}
...
AddEventHandler('playerConnecting', function (playerName,setKickReason, deferrals)
    ...
    for i = 1, #BanList, 1 do
        if ((tostring(BanList[i].license)) == tostring(license) or (tostring(BanList[i].identifier)) == tostring(steamID) or (tostring(BanList[i].liveid)) == tostring(liveid) or (tostring(BanList[i].xblid)) == tostring(xblid) or (tostring(BanList[i].discord)) == tostring(discord) or (tostring(BanList[i].playerip)) == tostring(playerip)) then
            setKickReason("[GS-AC] YYou've been permanently banned for: " .. BanList[i].reason)
            print("^6[GS-AC] - ".. GetPlayerName(_src) .." is trying to join the server, but he/she's banned ;(")
            CancelEvent()
        end
        ...
        for k,v in pairs(bannedtokens) do
            for i3 = 1, #tokens, 1 do
                if v == tokens[i3] then
                    setKickReason("[GS-AC] You've been permanently banned for: " .. BanList[i].reason)
                    print("^6[GS-AC] - ".. GetPlayerName(_src) .." is trying to join the server, but he/she's banned ;(")
                    CancelEvent()
                end
            end
    ...
    if GS_AC.AntiVPN then
        local _playerip = tostring(GetPlayerEndpoint(_src))
        if not GS_AC.WhitelistedIPS[_playerip] then
            deferrals.defer()
            Wait(0)
            deferrals.update("[GS-AC]: Checking and securing your connection...")
            PerformHttpRequest("https://blackbox.ipinfo.app/lookup/" .. _playerip, function(errorCode, _isusingvpn, resultHeaders)
                if _isusingvpn == "N" then
                    ...
                    print("^6[GS-AC]^0: The user ^0" .. playerName .. " ^1has been kicked for using a VPN, ^8IP: ^0" .. _playerip .. "^0")
                    deferrals.done("[GS-AC]: We've detected a VPN/Proxy in your machine. Please disable it or ask the server owners.")
                end
            end)
        end
    end
            ...
            if _entitytype == 0 then
                if inTable(WhitelistedPropList, model) == false then
                    if model ~= 0 and model ~= 225514697 then
                        ...
                        if entitiesSpawned[_src] > GS_AC.MaxEntitiesPerUser then
            ...
            if GetEntityType(entity) == 3 then
                if _entitytype == 6 or _entitytype == 7 then
                    if inTable(WhitelistedPropList, model) == false then
                        if model ~= 0 then
                            ...
                            CancelEvent()
                            entitiesSpawned[_src] = (entitiesSpawned[_src] or 0) + 1
                            if entitiesSpawned[_src] > GS_AC.MaxPropsPerUser then
                                LogDetection(_src, "Tried to spawn "..entitiesSpawned[_src].." props","model")
                                kickandbanuser(" Mass Prop Spawn", _src)
                                TriggerClientEvent("ZRQA3nmMqUBOIiKwH4I5:clearprops" , -1)
                            end
                ...
                if GetEntityType(entity) == 2 then
                    if _entitytype == 6 or _entitytype == 7 then
                        ...
                        if vehiclesSpawned[_src] > GS_AC.MaxVehiclesPerUser then
                ...
                elseif GetEntityType(entity) == 1 then
                    if _entitytype == 6 or _entitytype == 7 then
                        ...
                        if pedsSpawned[_src] > GS_AC.MaxPedsPerUser then
...
AddEventHandler("explosionEvent", function(sender, exp)
    if GS_AC.ExplosionProtection then
...
AddEventHandler('ptFxEvent', function(sender, data)
    ...
    if particlesSpawned[_src] > GS_AC.MaxParticlesPerUser then
...
AddEventHandler("weaponDamageEvent", function(sender, data)
    if GS_AC.UseESX and GS_AC.AntiTazePlayers then
        local _src = sender
        local _char = ESX.GetPlayerFromId(_src)
        if _char ~= nil and not GS_AC.WhitelistedJobs[_char.job.name] and data.weaponType == 911657153 or data.weaponType == GetHashKey("WEAPON_STUNGUN") then
...
AddEventHandler("giveWeaponEvent", function(sender, data)
    if GS_AC.AntiGiveorRemoveWeapons then
...
AddEventHandler("chatMessage", function(source, name, message)
    local _src = source
    if GS_AC.AntiBlacklistedWords then
        for k, word in pairs(GS_AC.BlacklistedWords) do
    ...
    if GS_AC.AntiFakeChatMessages then
...
AddEventHandler("clearPedTasksEvent", function(source, data)
    if GS_AC.AntiClearPedTasks then
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if GetCurrentResourceName() == resourceName then
        ...
        for k, v in pairs(GS_AC.BlacklistedModels) do
            table.insert(BlacklistedPropList, GetHashKey(v))
        end
        
        for k,v in pairs(GS_AC.WhitelistedProps) do
            table.insert(WhitelistedPropList, GetHashKey(v))
        end

        for k,v in pairs(GS_AC.BlockedExplosions) do
            table.insert(BlacklistedExplosionsList, v)
        end

        if GS_AC.AntiBlacklistedTriggers then
            for k, trigger in pairs(GS_AC.BlacklistedTriggers) do
        ...
        print([[
               ...
               ^8        ////////  ////////      //      //   //////                             
                                         ...
                                         ^2GS-AC Initialized. Enjoy!
]])   
        SetConvarServerInfo("AntiCheat:", "Protected by GS_AC v1")

        Citizen.Wait(1000)
        PerformHttpRequest("https://raw.githubusercontent.com/sakukaffe/gsacversion/main/gs_ac-version", function(error, latestVersion, headers)
            local currentVersion = "1.1"
            if tonumber(currentVersion) < tonumber(latestVersion) then
                print("                                             GS-AC" .. " ^1is outdated.\n                                           Current version: ^8" .. currentVersion .. "\n                                            Newest version: ^2" .. latestVersion .. "\n                                ^3Update^7: add Gaming_heart#1754")
            elseif tonumber(currentVersion) > tonumber(latestVersion) then
                print("                                GS-AC" .. " has skipped the latest version ^2" .. latestVersion .. "                      Either Github is offline or the version file has been changed")
            else
                print("                                          ^2GS-AC" .. " is updated. ("..currentVersion..")")
            end
        end, "GET")
    end
end)
...
kickandbanuser = function(reason, servertarget)
    if not IsPlayerAceAllowed(servertarget, "vbacbypass") and not IsPlayerAceAllowed(servertarget, "vbacadmin") and GS_AC.BanPlayers then
        local target
        local reason    = reason
        ...
        if target and target > 0 then
            ...
            if ping and ping > 0 then
                ...
                DropPlayer(target, "[GS-AC]:" .. reason)
            end
        end
    end
end
...
ban_user = function(source,token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason)
    if not IsPlayerAceAllowed(source, "vbacbypass") and not IsPlayerAceAllowed(source, "vbacadmin") then
        MySQL.Async.execute('INSERT INTO rented_vehicles (token,license,identifier,liveid,xblid,discord,playerip,targetplayername,reason) VALUES (@token,@license,@identifier,@liveid,@xblid,@discord,@playerip,@targetplayername,@reason)',{
...
loadBanList = function()
    MySQL.Async.fetchAll('SELECT * FROM rented_vehicles', {}, function (data)
        BanList = {}
        for i=1, #data, 1 do
...
LogDetection = function(playerId, reason,bantype)
    if not IsPlayerAceAllowed(playerId, "vbacbypass") and not IsPlayerAceAllowed(playerId, "vbacadmin") then
        ...
        local discordlogimage = "https://cdn.discordapp.com/attachments/801747400465186826/992459399631228998/1Komp_1_3-min.gif"
        
        local loginfo = {["color"] = "15158332", ["type"] = "rich", ["title"] = "A player has been banned by GS-AC", ["description"] =  "**Name : **" ..name .. "\n **Reason : **" ..reason .. "\n **ID : **" ..playerId .. "\n **IP : **" ..playerip.. "\n **Steam Hex : **" ..steamid .. "\n **Xbox Live : **" .. xbl .. "\n **Live ID: **" .. liveid .. "\n **Rockstar License : **" .. license .. "\n **Discord : **" .. discord, ["footer"] = { ["text"] = " © GS-AC | Gaming_heart#1754 - "..os.date("%c").."" }}
        if name ~= "Unknown" then
            if bantype == "basic" then
                PerformHttpRequest(GS_AC.GeneralBanWebhook, function(err, text, headers) end, "POST", json.encode({username = " GS-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "model" then
                PerformHttpRequest(GS_AC.EntitiesWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " GS-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"})
            elseif bantype == "explosion" then 
                PerformHttpRequest(GS_AC.ExplosionWebhookLog, function(err, text, headers) end, "POST", json.encode({username = " GS-AC", avatar_url = discordlogimage, embeds = {loginfo}}), {["Content-Type"] = "application/json"} )
            end
        end
        if GS_AC.ScreenshotPlayers then
            TriggerClientEvent("EuiAtK0QfujTpzWY0Mmp", playerId, GS_AC.ScreenShotWebhookLog)
        end
    end
end
...
--
-- GS-AC: ANTI INJECTION INSTALLATION FUNCS
--

RegisterCommand("gsacin", function(source)
    count = 0
    skip = 0
    if source == 0 then
        local randomtextfile = RandomLetter(12) .. ".lua"
        _antiinjection = LoadResourceFile(GetCurrentResourceName(), "GS-AC(AntiInjection).lua")
        for resources = 0, GetNumResources() - 1 do
            ...
            if _resourcemanifest then
                ...
                print("^1[GS-AC]: Anti Injection Installed on ".._resname)
                count = count + 1
            elseif _resourcemanifest2 then
                ...
                skip = skip + 1
                print("[GS-AC]: Skipped Resource: " .._resname)
            end
        end
        print("[GS-AC] Installation has finished. Succesfully installed Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
    end
end)

RegisterCommand("gsacun", function(source, args, rawCommand)
    if source == 0 then
        count = 0
        skip = 0
        if args[1] then
            ...
            for resources = 0, GetNumResources() - 1 do
                ...
                if resourcefile then
                    ...
                    if deletefile then
                        ...
                        Wait(100)
                        os.remove(_toremove)
                        print("^1[GS-AC]: Anti Injection Uninstalled on ".._resname)
                        count = count + 1
                    else
                        skip = skip + 1
                        print("[GS-AC]: Skipped Resource: " .._resname)
            ...
            print("[GS-AC] UNINSTALLATION has finished. Succesfully uninstalled Anti-Injection in "..count.." Resources. Skipped: "..skip.." Resources. Enjoy!")
        else
            print("[GS-AC] You must write the file name to uninstall Anti-Injection!")
        end
    end
end)
...
AddEventHandler('onResourceStop', function(resourceName)
 ...
 DropPlayer(-1, "[GS-AC]:" NICHT STOPPEN/NEUSTARTEN)
  os.exit()
end)