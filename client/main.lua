...
if GS_AC.Enable then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if GS_AC.AntiInfiniteStamina then
            ...
            if GS_AC.AntiExplosiveBullets then
            ...
            if GS_AC.AntiBlacklistedWeapons then
                for _,_weapon in ipairs(GS_AC.BlacklistedWeapons) do
            ...
            if GS_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
                _Wait(300)
            end
            if GS_AC.AntiNightVision then
                if GetUsingnightvision() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
                _Wait(300)
            end
            if GS_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            if GS_AC.AntiLicenseClears then
            ...
            if GS_AC.AntiBlacklistedTasks then
                for _,task in pairs(GS_AC.BlacklistedTasks) do
            ...
            if GS_AC.AntiBlacklistedAnims then
                for _,anim in pairs(GS_AC.BlacklistedAnims) do
            ...
            if GS_AC == nil or _evhandler ~= AddEventHandler or _onresstarting ~= "onResourceStarting" or _onresstart ~= "onResourceStart" or _onclresstart ~= "onClientResourceStart" or _antistop ~= GS_AC then
    ...
    Citizen.CreateThread(function()
        while GS_AC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if GS_AC.AntiVDM then
                    N_0x4757f00bc6323cfe(-1553120962, 0.0)
                end
                if GS_AC.DisableVehicleWeapons then
    ...
    Citizen.CreateThread(function()
        while GS_AC.AntiWeaponModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(10000)
            local VB_CIT_Wait = Citizen.Wait
    ...
    Citizen.CreateThread(function()
        ...
        while GS_AC.AntiCommandInjection do
            for _,cmd in ipairs(GetRegisteredCommands()) do
                if inTable(GS_AC.BlackListedCMD, cmd.name) then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
        if GS_AC.AntiPedRevive and GS_AC.UseESX then
            local distance = #(GS_AC.HospitalCoords - vector3(GetEntityCoords(PlayerPedId())))
            if distance < 50 then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
        if GS_AC.AntiVehicleSpawn then
            local _pcoords = GetEntityCoords(PlayerPedId())
            local isneargarage = false
            for _,v in pairs(GS_AC.GarageList) do
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if GS_AC.ClearPedsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if GS_AC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function(vehicles)
        if vehicles == nil then
            ...
            if GS_AC.ClearVehiclesAfterDetection then
                local vehs = GetGamePool('CVehicle')
                for _, vehicle in ipairs(vehs) do
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(5000)
        while GS_AC.OnScreenMenuDetection do
            if not ischecking then
                exports['bild']:requestScreenshot(function(data)
                    Citizen.Wait(1000)
                    SendNUIMessage({
            ...
            Citizen.Wait(GS_AC.OSMDCheckingTime)
        end
    end)
    ...
    RegisterNUICallback('menucheck', function(data)
        if GS_AC.OnScreenMenuDetection then
            if data.text ~= nil then     
                for _, word in pairs(GS_AC.BlacklistedMenuWords) do
    ...
    Citizen.CreateThread(function()
        while GS_AC.CheckPlayersMoney do
            Citizen.Wait(5000)
            local efectivo = nil
            local banco = nil
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        ...
        if _entityowner == GetPlayerServerId(PlayerId()) or args[2] == -1 and GS_AC.AntiAimbot then
        ...
        if GS_AC.DeleteBrokenCars then
        ...
        if GS_AC.AntiSuicide then
            if name == 'CEventNetworkEntityDamage' and args[2] == -1 then
                if GS_AC.usingBuild2189 then
    ...
    if GS_AC.AntiResourceStartorStop then
